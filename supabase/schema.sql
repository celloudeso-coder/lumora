-- ============================================================
-- LUMORA GROUP — Schéma Supabase
-- Exécutable tel quel dans l'éditeur SQL Supabase (ou via CLI :
-- supabase db push / psql). Voir ARCHITECTURE.md pour le contexte.
-- ============================================================
...
create table if not exists public.class_slots (
  id           uuid primary key default gen_random_uuid(),
  title        text not null,                       -- ex. « Pilates Mat débutant »
...
create table if not exists public.bookings (
  ...
  service       text,                               -- Beleza : « Massage relaxant », etc.
  ...
  created_at    timestamptz not null default now(),
  -- Cohérence : un créneau pour Pilates, une date souhaitée pour Beleza.
...
-- ------------------------------------------------------------
-- 7. STORAGE : images de couverture des articles
-- ------------------------------------------------------------
